...
echo "[03] Building vLLM from source for ROCm gfx1151..."
echo "  NOGPU: ${NOGPU}"
...
echo "[03a] Creating get-torch-dir.py helper script..."
...
# Step 1: Clone vLLM
echo "[03b] Checking vLLM repository..."
cd "${WORK_DIR}"
if [ ! -d "${VLLM_DIR}" ]; then
    echo "  Cloning vLLM..."
...
echo "[03b] Configuring vLLM to use existing torch installation..."
...
echo "[03c] Patching CMakeLists.txt for CPU-only build..."
if [ "${NOGPU}" = "true" ]; then
...
echo "[03d] Setting build environment for gfx1151..."
...
export BUILD_FA="0"
# Set CMAKE_HIP_ARCHITECTURES directly to bypass GPU detection
...
echo "[03e] Installing build dependencies and vLLM requirements..."
# Install vLLM ROCm requirements (includes common.txt and AMD-specific deps)
pip install --no-cache-dir -r "${VLLM_DIR}/requirements/rocm.txt"
# Install/upgrade additional build tools (upgrade ninja to latest to avoid lexing errors)
pip install --no-cache-dir --upgrade 'cmake<4' wheel 'setuptools<80' pybind11 Cython build "setuptools-scm>=8" grpcio-tools einops pandas psutil ninja

# Step 4: Build and install vLLM using setup.py develop
echo "[03f] Building and installing vLLM using setup.py develop..."
