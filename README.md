...
When installing PyTorch from the nightly repo, it pins specific versions of `rocm-sdk-core` and `rocm-sdk-libraries-gfx1151`, but may leave `rocm-sdk-devel` at a different version. We explicitly synchronize all three components:
...
This ensures all SDK components match the version expected by PyTorch.
...
4. **Check library symlinks:**
   ```bash
   find /opt/rocm/lib -type l -exec test ! -e {} \; -print
   ```
   This command will list any broken symlinks. Output should be empty if all symlinks are valid.
